...
final class SceneSplitTests: XCTestCase {
    ...
    func test상한_초과_본문_타일링_무손실() {
        assertTiling(longProse(atLeast: 10000))
    }

    func test헤딩_있는_장편_타일링_무손실() {
        let body = "# 1장\n" + longProse(atLeast: 6000) + "\n# 2장\n" + longProse(atLeast: 4000)
        assertTiling(body)
    }

    func test개행_뭉치_보존() {
        // 시뮬레이션에서 실제로 잃었던 케이스 — 헤딩 사이 빈 줄.
        let body = longProse(atLeast: 3000) + "\n\n\n" + longProse(atLeast: 3000)
        assertTiling(body)
    }

    func test공백_꼬리_보존() {
        assertTiling(longProse(atLeast: 2000) + "   \n  ")
    }

    // MARK: - 크기 보장

    func test모든_세그먼트가_상한_이하() {
        let body = longProse(atLeast: 30000)
        let outline = DocumentOutline.parse(body)
    ...
    func test같은_입력은_같은_경계() {
        ...
        // 바뀌어 매 실행 전체 재요약이 된다.
        let body = longProse(atLeast: 10000)
