...
- **개발 편집 평면 (제품화 격차)**: 전체 백엔드 검토 —
  **[docs/novel-analysis-gap-review.md](docs/novel-analysis-gap-review.md)**.
  현 백엔드는 엔티티·지식·그래프(사실 추적)는 깊으나 개발 편집(씬 목적·갈등·
  페이싱·감정 아크·세계관 바이블·연속성 확장)이 비어 있다. 우선순위: 결정적
  즉효(산문 건강·감정/긴장 아크·연속성 확장·목소리 구별도) → 세계관 엔티티
  일반화(`CharacterCard`→`StoryEntity`) → 씬 목적·갈등 LLM 추출. Fictionary 38
  요소·AutoCrit 페이싱·Reagan 6 감정 아크 기준. 전 격차 §5 체크리스트 통과.
