...
- **`serialize()`가 매 키 입력마다 문서 전체를 돈다** — 88k자에서 5ms(폰트 수정
  후), 30만 자면 ~17ms로 프레임 예산을 다시 먹는다. 증분 직렬화(편집된 문단만
  재직렬화, 단일 문단 = 0.00ms 측정)가 정공법 — CLAUDE.md §2-3. docs/editor-perf.md.
...
3. **헤딩 없는 원고의 씬 분할** — **2026-07-15 실측 결함**: 「날개」 22,521자가
   `# 날개` 헤딩 하나뿐이라 전체가 씬 1개가 되고, `maxSceneCharacters=1500`에
   걸려 **앞 6.7%만 이해된다** (나머지 21,000자는 요약·사건 어디에도 없음).
   장·작품 요약도 씬 2개 미만이라 전파되지 않는다. §8의 "골격 = 헤딩 구조"
   전제가 헤딩을 안 쓰는 원고(웹소설·단편)에서 무너진다 — 사건 로그 품질
   이전의 문제. 선택지: (a) 헤딩이 없으면 빈 줄·길이 기반 씬 분할 폴백
   (b) 긴 씬을 상한 단위로 쪼개 하위 노드로 (c) 사용자에게 헤딩을 권유(회피).
   (a)가 유력하나 Pos·KV·톰스톤에 파급 — 착수 전 설계 필요. docs/m6-events.md.
