...
- **모델 프리셋** (UI 이름 nano/air/pro): `Qwen2.5-1.5B-4bit`(~1GB, 기본) /
  `Qwen2.5-3B-4bit`(~1.9GB) / `Qwen3.6-35B-A3B-4bit`(~20GB).
...
**감지 깔때기 (결정적 우선, CLAUDE.md §2-5):**

1. 후보 추출 (결정적): 고유명사꼴 토큰 빈도 (조사 휴리스틱 제거: "서연이가/서연을"
   → "서연") + 대화 인접 패턴 ("~가 말했다", 따옴표 앞뒤 주어) 가중.
2. 임계 통과 (예: 3+ 씬에서 5+ 언급) → **사용자 확인** (비모달 — 사이드바 배지
   "'서연'을(를) 인물로 등록할까요?"). 거부 목록 유지 — 같은 후보 재질문 금지.
...
4. 바이블 노출 수위 — 이해를 보여줄수록 신뢰가 오르나 소음도 는다. 기본 접힘 +
   요청 시 상세가 가설.
5. 한국어 형태소 분석 도입 여부 — 조사 휴리스틱(§7)의 오탐이 실사용에서 문제되면
   `NLTagger` 한국어 토크나이즈부터 검토 (외부 무거운 형태소기는 최후).
