...
2. **이해 경로** (유휴 시, 백그라운드): 유휴 감지 → 더티 씬 추출·요약 →
   KnowledgeStore 갱신(append) → 지식 세대 +1 → KV 프리픽스 프리웜 (§12).
...
- **아웃라인 트리**: 마크다운 헤딩 `#`(부/장) `##`(장/절) `###`(씬)을 결정적으로
  ...
  헤딩 없는 문서는 전체가 단일 씬 (저널이 자연히 이 경우).
...
- 모드는 `EntryKind` + 바이블 유무로 자동 선택, Settings에서 수동 오버라이드.
  Story의 "웜"은 KV 프리웜 적중 (§12) — 콜드는 지식 세대가 방금 바뀐 직후뿐이다.
...
1. **프리픽스 KV (A+B)**: 지식 세대가 바뀔 때만 재프리필. **이해 경로의 마지막
   단계가 새 A+B를 백그라운드에서 프리웜**하므로, 예측 시점엔 이미 따뜻하다 —
   "백그라운드가 준비하고 예측은 조립만"의 KV 레벨 구현.
...
- [x] A+B 프리픽스 KV 프리웜 (이해 경로 마지막 단계) — 패스 완주 신호 →
      예측과 동일 조립·동일 생성 경로(maxTokens=1)로 프리필. 실기기 웜 TTFC
      확인은 E2E 절차로 (docs/m6-knowledge.md 프리웜 절)
- [x] 바이블 패널 UI (열람·수정·`locked`·미확인 검토) — 팝오버 확장으로 마감
      (별도 패널 승격은 이득 없어 기각, docs/m6-dialogue.md): 카드별 자동 이해
      열람(예측과 같은 질의) · 소개 직접 수정 시 자동 잠금 + 잠금 토글 ·
      후보 전부 나열. 이해 타임라인(`KnowledgeTimelineView`)은 사건·델타 열람.
      파생 지식 직접 수정은 안 연다 — 원문이 유일한 진실 (§2-1)
...
3. **씬 크기에 상한이 없다 → 이해 상한을 넘는 만큼 조용히 사라진다**
   ...
   **설계 완료 (2026-07-16): [docs/m6-scene-split.md](docs/m6-scene-split.md)**
   — 문장을 바닥 단위로, 경계는 내용 정의(CDC)로. 이해 10.2%→100%,
   파급 반경 평균 1.2·최악 9 (greedy는 헤딩 없는 원고에서 최악 23/28).
   미착수 — 스키마 v3 + 커서 근처 우선 처리가 함께 가야 한다(개츠비 174회 재구축).
...
- **LCP 증분 프리필** (`PromptCache`) — 직전 프롬프트와의 최장 공통 접두까지 KV를
  ...
  병목이다.
- **A+B 프리픽스 프리웜** (M6) — 이해 패스 완주 직후, 예측과 **동일한 조립·생성
  경로**(maxTokens=1)로 새 A+B를 미리 데운다. *의도*: 지식 세대가 바뀌면 헤더
  첫 글자부터 LCP가 죽는다 — 그 콜드 프리필을 유휴 시간으로 옮긴다 ("백그라운드가
  준비하고 예측은 조립만"의 KV 레벨 구현).
...
- **스토리 바이블 팝오버** — 툴바 "소설" 배지 → 장르·인물 카드 편집 + 후보 검토 +
  **자동 이해 열람**(상태·최근 사건·말투, 예측과 같은 질의). *의도*: AI가 이해한
  모든 것을 사용자가 보고 고칠 수 있어야 한다 (§1-5). 파생 지식 직접 수정은 안
  연다 — 원문과 어긋난 채 다음 패스가 덮는다 (§2-1).
- **이해 타임라인** (`KnowledgeTimelineView`) — 툴바 분기 아이콘 → 씬 분할·사건·
  델타·미이해 구간을 gitgraph꼴로 담화 순서 열람. *의도*: 지식이 JSON에만 있으면
  §1-5가 깨진다. "헤딩 없는 장편이 씬 하나로 잘리는" 결함도 여기서 드러난다.
