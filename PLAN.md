...
1. ~~**MINT(Ternary-Bonsai-27B, 2비트)가 한국어 장편을 감당하는가**~~
   **해소(2026-08-22 측정, [docs/model-lineup-bench.md](docs/model-lineup-bench.md))
   ...
   지연·재사용은 구조적 문제라 남는다. **기본 모델 교체(Basil 승격)는 사용자 결정** —
   데이터상 Basil 전환이 지연·재사용·오염 면에서 근거를 갖는다.
...
2. **라인업 v2의 체급이 좁다** — 8.5 / 16.9 / 20GB. "체급이 비슷하되 용도가 다른"
   ...
   각 모델의 자리 확정·`detail` 문장 작성은 남아 있다. 부수 문제: 1GB급이 피커에서
   빠져 **첫 실행 다운로드가 8.5GB**다 — 첫 실행 부담이 문제가 되면 경량 대안 복귀 또는 "모델 선택 전 대기"
