...
3. "폴더 = 작품(장별 문서)" 전환 — 초장편 UX엔 유리하나 아웃라인·Pos·KV 전략
   전반에 파급. 1작품 1문서의 실사용 한계가 보일 때 재론.
4. 지식 사이드카 SQLite 전환 시점 (§6 전환 조건 도달 여부).
5. 바이블 노출 수위 — 이해를 보여줄수록 신뢰가 오르나 소음도 는다. 기본 접힘 +
   요청 시 상세가 가설.
6. ~~한국어 형태소 분석 도입 여부~~ **해소(2026-07-14)**: `NLTagger`는 한국어
