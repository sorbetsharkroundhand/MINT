...
- **모델 프리셋 — 라인업 v2** (2026-07-17): 체급 사다리(nano/air/pro)를 버리고
  **허브 이름 mint/basil/peppermint**로 간다. 근거: 셋 다 27~35B급 이해에 8.5–20GB로
  ...
  | UI | 저장소 | 크기 | 성격 |
  |---|---|---|---|
  | **MINT** | `prism-ml/Ternary-Bonsai-27B-mlx-2bit` | ~8.5GB | Qwen3.5-27B **밀집**, 2비트 삼진(~1.71bpw). 토큰마다 전 가중치를 읽어 대역폭이 MoE보다 크다. **2026-08-22 벤치로 기본 부적합 판정 — 기본값에서 해제 (#40)** |
  | **Basil** (기본) | `mlx-community/GLM-4.7-Flash-4bit` | ~16.9GB | MoE 총 30B·활성 ~3B (`glm4_moe_lite`, mlx-swift-lm 3.31.3+). **웜 KV 재사용 884/885 tok · 웜 TTFC ~0.05s** — §12 전략이 온전히 유효한 유일한 라인업 |
  | **Peppermint** | `mlx-community/Qwen3.6-35B-A3B-4bit` | ~20GB | MoE 활성 ~3B. 콜드 1.41s로 빠르나 재사용 불가(Qwen3.6 하이브리드) |
  ...
  **기본 모델 = Basil (#40, 결정안 1+2)**: 신규 사용자만 Basil로 시작하고 기존
  저장 modelID는 유지된다. 첫 실행 모델 선택 시트(ContentView InitialModelPicker)
  가 자동 다운로드 전에 크기·성격을 보여준다 — "조용한 16.9GB" 방지.
...
### 5.2 프로젝트 저장과 비파괴 이관 (0.2.0, #102)

- `ProjectStore`는 명시적으로 주입한 `Projects/` 루트를 소유하는 actor다. UI나
  예측 경로에서 파일을 읽지 않으며, 실제 앱의 EntryStore 연결은 #103 이후에 한다.
  같은 루트의 협조하는 저장소 인스턴스/프로세스는 파일 잠금으로 직렬화한다.
- `project.json`에는 문서 메타데이터·상대경로·SHA-256만 둔다. 본문은
  `Documents/<문서 UUID>/<본문 SHA-256>.md`, 메모는 `Notes/` 아래의 같은 구조다.
  변경된 본문만 새 파일에 쓰고 재확인한 뒤 manifest를 원자 교체한다. 파일별 덮어쓰기 후
  manifest만 마지막에 쓰는 방식은 중간 실패 시 이전 본문을 훼손하므로 사용하지 않는다.
- 이전 manifest는 `previous-project.json`에 보존한다. 손상 시 읽기 전용 복구 API로
  직전 유효 원고를 얻는다. 자동 빈 프로젝트 생성이나 손상 파일 덮어쓰기는 하지 않는다.
  이전 본문 파일의 GC는 이 이슈에서 하지 않는다.
- 상대경로는 문서 종류·ID·해시에 맞는 경로만 허용하고 심볼릭 링크를 거부한다.
  읽을 때 파일 해시·UTF-8·스키마·중복 문서 ID를 검증한다.
- migration은 `entries.json`을 읽기 전용 스냅샷으로 디코딩한다. 원본 바이트 전체를
  `UserData/`에 보존하므로 폴더·날짜·사용자 인물 카드·수정 및 미지 필드도 남는다.
  미래의 User Canon 저장 위치는 `UserData/canon.json`으로 예약한다.
- 기존 앱이 관리하는 `images/` 전체를 `Assets/`에 해시별로 복사한다. 본문 참조를
  고치지 않고 manifest의 원래 상대경로→사본 경로 매핑으로 해석한다. 외부 파일/원격
  참조는 원문에 남으며 자동으로 읽거나 내려받지 않는다.
- 원본·asset·최초 모드/제목 fingerprint별 receipt를 `Projects/Migrations/`에 기록한다.
  target을 재검증한 뒤 `active-project.json`을 마지막에 원자 교체한다. 재이관은 유효
  receipt의 프로젝트를 재사용해 이관 후 편집을 보존한다. target 손상 시 기존 파일을
  남기고 원본에서 별도 프로젝트를 생성한다. 자동 추론의 `Intelligence/` 삭제는 무관하다.
- 구현 검증은 격리 임시 디렉터리의 원본 바이트 비교, 쓰기 실패, 재이관, 손상 복구와
  경로 차단에 집중한다. 앱 shell·새 UI·Canon 의미 모델은 이 이슈의 범위가 아니다.
...
### 5.4 문서 중심 앱 셸 (0.2.0, #103)

- `WorkspaceShellView`는 탐색기·에디터·선택 도구를 슬롯으로 받아 배치만 담당한다.
  창 폭에 따라 탐색기를 200–360pt로 제한하고 본문에 최소 560pt를 남긴다.
  접기나 일시적인 창 축소는 사용자가 저장한 탐색기 폭을 덮어쓰지 않는다.
- `ProjectNavigatorView`는 기존 문서 트리의 검색·이름 변경·드래그 이동을 재사용한다.
  바이블·서사·AI 컨텍스트를 열어도 왼쪽 문서 목록은 유지한다. 도구는 넓은 창에서는 오른쪽,
  좁은 창에서는 편집기 아래에 배치해 입력 중인 커서·고스트를 가리지 않는다.
- 본문은 `MintTheme.editorSurface`의 불투명 표면이다. Material은 주변 도구에만
  적용하며 투명도 감소 설정에서는 불투명 표면으로 대체한다. 커서 줄 강조와
  AppKit 편집기·IME·고스트 처리 경로는 그대로 유지한다.
- 이 단계의 데이터 어댑터는 `EntryStore`다. `ProjectStore`의 영속 프로젝트
  선택·세션 수명·모드 전환은 #104에서 연결하며, 자동 이관을 시작하지 않는다.
...
**인물 카드 스키마** (프롬프트 주입 형태, §11 — 2026-08-22 산문형 개편):

```
서연 (연이, 서 대리) — 신중하고 직설적이다. 목표는 아버지 실종의 진상.
지금 병원에 있다. 마음은 불안이다. 민준을 의심한다 — 약이 바뀌었다.
최근에 병원에서 퇴원했다.
```

- **레코드 덤프 폐지**: `상태@커서: 위치=병원` 같은 `필드=값` 인포박스는
  continuation 베이스 모델에 도메인 외 포맷이다. 소설 코퍼스에 맞춰진 모델일수록
  서술형 문장을 어투 누수 없이 흡수한다(NovelAI lorebook 커뮤니티 실측, CHIRON의
  QA시트 > 속성 나열 — EMNLP 2024 Findings). 데이터 구조(StateDelta 닫힌 필드)는
  그대로, **렌더링만** 산문으로 바꿨다.
- 사망은 `N은 이미 죽어 있다.` 한 문장으로 승격 — 일관성 실탄(생사)을 모델이
  가장 못 벗어나는 형태로 박는다. 생존 같은 기본값은 침묵한다(토큰당 품질).
...
```
[A 고정 헤더]   작품 메타(제목·장르·종류) + 인물 카드(산문형, ≤ 700자)
[B 지식 블록]   흐름 사건(회상 중) → 씬/장 요약(살라이언스순 선별·문서순 정렬)
                → 지난 줄거리(작품 요약, 남는 예산일 때만) — 합계 ≤ 600자
[그라운딩]      지금 장면(제목·유형·장소·시점) + 이 장면의 사람들 + 대화 모드
                — 커서가 씬을 넘을 때만 변한다 (맨 뒤, KV 프리픽스 보호)
[C 최근 원문]   커서 앞 원문, 남는 예산 전부 — 매 키 입력 변화
```

- **3계층 역할 분리** (2026-08-22 개편): ① 일관성 계층(A 카드 상태·앎 — primacy,
  프롬프트 앞) ② 그라운딩 계층(지금 장면·동석 인물 — recency, 커서 직전) 
  ③ 스타일 계층(시점 명시·대화 모드 말투 예문). 다음 문장 예측의 실탄은
  "지난 줄거리"가 아니라 "지금 이 공간에 누가 있는가"다.
- **살라이언스 선별** (Indexter pairwise event salience): B 후보 씬을 담화 거리
  대신 인덱스 공유로 점수 매긴다 — 인과 선행 +30 · 같은 장소 +20 · 동석 인물
  +10(상한 2명), 기본 5점 − 거리×0.25. 같은 점수면 가까운 쪽(결정적 타이브레이크).
  재료는 전부 NarrativeGraph 질의(`sceneSalienceScores`)라 예측 경로에서 LLM 없이
  O(사건+간선)로 산다. 실제 줄 순서는 문서 순서 복원 — 출력 안정성 유지.
- **작품 요약 강등**: `지난 줄거리`(작품 요약)를 B 맨 뒤·남는 예산으로. 일관성
  오류의 최다 분포가 factual·temporal이라(ConStory-Bench 2026) state·장면 지식이
  먼저다. 회상 집필 중엔 여전히 아예 빠진다(미래 누출 차단).
- **랭킹 점수** (카드): 활성 엔티티 겹침(최근 창에 언급된 인물) > 문서 순서
  (주인공 추정). Pin은 예산을 이긴다.
- **중복 제거**: C 창 안에 이미 보이는 사실은 B에서 뺀다 — 토큰당 품질 원칙.
- **시점 차단**: 모든 B 항목은 `state_at(커서)`·`upTo(커서)` 질의 결과만.
  커서 뒤 텍스트·미래 지식은 절대 주입하지 않는다.
- **예산은 문자 기반 줄 단위** — 통짜 prefix 자름은 마지막 줄을 반쪽 내고 리포트와
  어긋난다. 초과 줄부터 통째로 탈락. 토큰 기준 전환은 §10 미해소 과제.
...
- ⚠️ **적용 한계 (2026-08-22 실측)**: LCP trim은 순수 어텐션 모델에서만 작동한다 —
  Qwen3.5/3.6 계열 하이브리드(선형어텐션 레이어)는 `MambaCache`(`isTrimmable=false`
  상속)를 써서 `canTrimPromptCache`가 false → 재사용 0으로 전체 프리필 폴백.
  MINT(Ternary-Bonsai)·Peppermint(Qwen3.6)에서 웜 TTFC가 콜드와 같은 이유다.
  GLM-4.7-Flash(Basil)는 trimmable — 웜 0.05s로 §12 전략이 온전히 유효
  ([docs/model-lineup-bench.md](docs/model-lineup-bench.md)). 모델 선택의 핵심
  기준 중 하나가 됐다.
...
- ~~취소된 생성이 KV 캐시를 버린다~~ **해소(2026-08-22)**: catch가 CancellationError와
  ...
  짝을 맞추고, GPU 오류 등만 abandon한다. 내부 생성 Task 종료를 기다린 뒤 commit하고
  busy 가드를 내려, 이후 부모 취소가 안전한 캐시를 다시 abandon하지 않는다. 리플레이
  벤치에서 평균 763/764토큰 재사용과 웜 TTFC 0.01초(콜드 0.52초)를 확인했다.
- 코드 주석의 `PLAN §N` 일부는 구판 번호 — 만나는 대로 부록 A 기준으로 갱신.
- ~~MINTBench 릴리즈 CLI 간헐 세그폴트~~ **해소(2026-08-23)**: MLX 내부 생성
  Task를 모든 조기 종료 경로에서 cancel 후 await하고, 모델 교체도 기존
  컨테이너 사용권이 모두 반환된 뒤 시작한다. Qwen2.5-3B 내부 생성 시작 이후
  취소→후속 생성 20/20과 독립 프로세스 10/10(각 20회, 총 200/200), 첫 청크
  조기 종료, 리플레이 웜 KV 재사용 763/764토큰을 확인했다. EXIT=11과 빈 후속
  생성은 없었다 (`docs/cancellation-stress-2026-08-23.md`).

**열린 질문 (결정 전 — CLAUDE.md §5 체크리스트로 평가)**

1. ~~**MINT(Ternary-Bonsai-27B, 2비트)가 한국어 장편을 감당하는가**~~
   **해소(2026-08-22 측정, [docs/model-lineup-bench.md](docs/model-lineup-bench.md))
   — 기본 부적합 판정.** 리플레이 벤치(1200자·컷 12): 웜 TTFC 4.24s(예산의 ~9배,
   KV 재사용 0 — 아래 §12 하이브리드 한계), 프리필 ~7ms/tok, 제안에 중국자 유입
   ("亮히" 등 — 1.71bpw가 한국어를 먼저 무너뜨린다는 우려의 직접 증거),
   어절 적중률 8%. 사고 태그 오염은 `stripContinuationThinking`으로 수정했으나
   지연·재사용은 구조적 문제라 남는다. **해소(#40, 2026-08-25): Basil을 신규
   기본으로 승격**했다 — 기존 사용자는 저장 modelID 유지, 첫 실행 모델 선택 시트가
   16.9GB 다운로드 전에 노출된다 (결정안 1+2 동시 적용).
...
2. **라인업 v2의 체급이 좁다** — 8.5 / 16.9 / 20GB. "체급이 비슷하되 용도가 다른"
   ...
   아키텍처 사실만 적고 품질·지연 서열은 비워 둠). **1차 실측 완료(2026-08-22)**:
   Basil만 §12 KV 전략+지연 예산 충족(웜 0.05s·재사용 884/885), Peppermint는
   MoE라 콜드 1.41s로 빠르나 재사용 불가(Qwen3.6도 하이브리드), MINT는 최하.
   각 모델의 자리 확정·`detail` 문장 작성은 남아 있다. 부수 문제(첫 실행 다운로드):
   **#40 결정안 2로 해소** — 첫 실행 모델 선택 시트가 기본 다운로드 전에 크기를
   보여주고, 경량 대안은 Settings 수동 입력으로 계속 이용 가능.
   기존 사용자는 저장된 modelID를 유지하므로 강제 재다운로드 없음.
   **경량 슬롯 교체(#63 해소, 2026-08-25)** — A.X-4.0-Light(meetLog 4bit)가 게이트
   통과(웜 0.03s·재사용 460/461·어절 적중 17% vs 구 후보 8%, Apache-2.0)로
   `ModelPresets.ax4Light` 반입. 실사용 수락률 대조(#41): MINT 시대 fast 27.8% vs
   story 7.1%·p95 5.05s — 지연이 수락을 깎는 방증. **Basil 승격 후 축적 재대조가
   남은 후속.**
...
| §5.2 현행 편집·서사 분석 모델 (#101 시점) | §5.3 (#102에서 저장 모델 §5.2 추가) |
...
- **시점 차단** — 모든 B 항목은 커서 이전 씬의 `state_at`·`upTo` 질의 결과만.
  *의도*: 3장을 고치는 중에 9장의 결말이 새어 들면 안 된다 (CLAUDE.md §2-4).
  씬 위치 비교라 결정적으로 성립한다.
- **상태 산문 절** (M6-5b, 2026-08-22 개편) — 카드에 fold한 상태를
  `지금 병원에 있다. 마음은 불안이다.` 꼴의 자연 문장으로 편다. *의도*:
  `필드=값` 레코드는 continuation 베이스 모델에 OOD 포맷이라 어투 누수·무시가
  잦았다. 사망은 `N은 이미 죽어 있다.`로 승격해 죽은 인물 등장을 프롬프트 수준에서
  막는다 (일관성 > 유창성).
- **앎 태도 문장** (v4, 2026-08-22 개편) — `사실(태도)` 튜플 대신
  `서연은 숨기고 있다 — ….` 꼴. 오해는 "잘못 믿고 있다"로 렌더링해 모델이
  거짓을 진실로 흡수하는 것을 막는다.
- **동석 인물 그라운딩** (2026-08-22) — "이 장면의 사람들: …" 한 줄. 씬 발화
  (커서 이전)·씬 사건 참여자 합집합 (`sceneCohabitants`). *의도*: 다음 문장
  예측에는 작품 요약보다 "지금 이 공간에 누가 있는가"가 정보량이 크다.
- **살라이언스 씬 선별** (2026-08-22) — B 후보를 거리순 대신 인과 선행·같은 장소·
  동석 인물 점수순으로 선별(`sceneSalienceScores`, Indexter 근거). *의도*: 원인이
  된 과거 장면은 멀어도 다음 장면 예측에 더 진한 신호다.
