...
- [ ] 바이블 패널 UI (열람·수정·`locked`·미확인 검토) — v0: 팝오버에 후보 확인 배너
      ...
      미이해 구간을 담화 순서로 **열람**. 수정은 아직 — §1-5의 "고칠 수 있어야"는 미완
...
- **`serialize()`가 매 키 입력마다 문서 전체를 돈다** — 88k자 5ms · 30만 자 ~17ms.
  2026-07-16에 다른 O(n)(drawBackground·마커 검사)을 범위 제한으로 없앤 뒤
  **남은 유일한 O(n)**이다. 증분 직렬화(단일 문단 = 0.00ms 측정)가 정공법이나
  캐시 버그 = 원고 손상이라 "이어붙인 결과 == 원문" 테스트를 먼저 세우고 착수한다
  — CLAUDE.md §2-3. 안·위험 비교: docs/editor-perf.md.
- 에디터 자동 테스트 부재 — 위 O(n) 수정들을 스크래치 벤치로만 검증했다
  (drawBackground 범위 제한의 코드 블록 배경은 육안 미확인).
